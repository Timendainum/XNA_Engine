...
namespace ThreeDWindowsGameLibrary.Simulation
{
	...
	public class DebugDrawer : DrawableGameComponent, Jitter.IDebugDrawer
	{
		...
		ICameraEnabledGameScreen Screen;


		public DebugDrawer(Game game, ICameraEnabledGameScreen screen)
			: base(game)
		{
			Screen = screen;
		}
		...
		public void DrawLine(JVector p0, JVector p1, Color color)
		{
			...
			LineList[lineIndex - 2].Position = VectorHelper.ToXNAVector(p0);

			LineList[lineIndex - 1].Color = color;
			LineList[lineIndex - 1].Position = VectorHelper.ToXNAVector(p1);
		}
		...
		public void DrawTriangle(JVector p0, JVector p1, JVector p2, Color color)
		{
			...
			TriangleList[triangleIndex - 2].Position = VectorHelper.ToXNAVector(p0);

			TriangleList[triangleIndex - 1].Color = color;
			TriangleList[triangleIndex - 1].Position = VectorHelper.ToXNAVector(p1);

			TriangleList[triangleIndex - 3].Color = color;
			TriangleList[triangleIndex - 3].Position = VectorHelper.ToXNAVector(p2);
		}
