...
namespace HexTileMapTestGame
{
	...
	/// </summary>
	public class HexTileTestGame : Microsoft.Xna.Framework.Game
	{
		...
		protected override void LoadContent()
		{
			...
			Texture2D newTexture = Content.Load<Texture2D>(@"TileSets\part3_tileset");
		...
		protected override void Update(GameTime gameTime)
		{
			...
			if (ks.IsKeyDown(Keys.Left))
			{
				Camera.Location.X = MathHelper.Clamp(Camera.Location.X - 2, 0,
				    (hexMap.Width - squaresAcross) * HexTile.StepX);
			}
			...
			if (ks.IsKeyDown(Keys.Up))
			{
				Camera.Location.Y = MathHelper.Clamp(Camera.Location.Y - 2, 0,
				    (hexMap.Height - squaresDown) * HexTile.StepY);
			}
